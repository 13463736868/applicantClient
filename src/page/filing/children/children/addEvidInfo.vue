      ...
      <Col span="22" offset="1">
        ...
        <Row class="_labelFor" v-if="fileList.length !== 0">
          ...
          <Col span="16" class="_input">
            <p class="_item" v-for="item in fileList" :key="item.id"><span class="hand" :title="item.filename" @click="seeFile(item.filepath)">{{item.filename.length > 50 ? item.filename.substr(0, 50) + '...' : item.filename}}</span><Icon @click="delFile(item.id)" class="_del hand" type="close-circled"></Icon></p>
          </Col>
