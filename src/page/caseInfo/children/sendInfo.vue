    ...
    <alert-see-info :alertShow="alertShowSub" @alertConfirm="seeInfoSave" alertTitle="查看">
      <div v-if="alertShowSub">
        ...
        <Row v-if="alertObj.fileShow">
          ...
          <Col span="16">
            <p v-for="item in alertObj.fileList" :key="item.id" class="p5 alert_file"><span class="hand" style="color:#126eaf" title="点击查看" @click="resSeeFile(item.filepath)">{{item.filename.length > 25 ? item.filename.substr(0, 20) + '...' : item.filename}}</span></p>
          </Col>
        </Row>
      </div>
    </alert-see-info>
