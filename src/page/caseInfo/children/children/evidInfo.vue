          ...
          <Col span="22" offset="1">
            ...
            <p>
              <Row>
                ...
                <Col span="17">
                  <p class="_file p0" v-for="(item, index) in infoData.fileUploads" :key="index" :title="'点击查看: '+item.filename" @click="seeFile(item.filepath)">{{item.filename.length > 30 ? item.filename.substr(0, 30) + '...' : item.filename}}<b v-if="infoData.state === 2" class="ml10" style="color:#ed3f14;">（请注意：该证据非证据原件）</b></p>
                </Col>
              </Row>
