      ...
      <Col span="22" offset="1">
        ...
        <Row class="_labelFor" v-if="fileList.length !== 0">
          ...
          <Col span="16" class="_input">
            <p class="_item p0" v-for="item in fileList" :key="item.id"><span class="hand" :title="item.filename" @click="seeFile(item.filepath)">{{item.filename.length > 30 ? item.filename.substr(0, 30) + '...' : item.filename}}</span><Icon @click="delFile(item.id)" class="_del hand" type="close-circled"></Icon></p>
          </Col>
...
export default {
  ...
  computed: {
    addFileBtn () {
      if (this.fileList.length === 0 || this.data.name === '' || this.data.state === null || this.data.memo === '' || this.addBtnSwt === true) {
        return true
      } else {
        return false
      }
